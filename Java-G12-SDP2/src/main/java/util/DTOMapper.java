...
public class DTOMapper
{
	...
	public static AddressDTO toAddressDTO(Address address)
	{
		...
		return new AddressDTO(address.getId(), address.getStreet(), address.getNumber(), address.getPostalcode(),
				address.getCity());
	}
	...
	public static UserDTO toUserDTO(User user)
	{
		...
		return new UserDTO(user.getId(), user.getFirstName(), user.getLastName(), user.getEmail(),
				user.getPhoneNumber(), user.getBirthdate(), toAddressDTO(user.getAddress()), user.getRole(),
	...
	public static SiteDTOWithoutMachines toSiteDTOWithoutMachines(Site site)
	{
		...
		return new SiteDTOWithoutMachines(site.getId(), site.getSiteName(), toUserDTO(site.getVerantwoordelijke()),
				site.getStatus(), toAddressDTO(site.getAddress()));
	}
	...
	public static List<SiteDTOWithoutMachines> toSiteDTOsWithoutMachines(List<Site> sites)
	{
		...
		return sites.stream().map(DTOMapper::toSiteDTOWithoutMachines).collect(Collectors.toList());
	}
	...
	public static SiteDTOWithMachines toSiteDTOWithMachines(Site site)
	{
		...
		Set<MachineDTO> machineDTOs = site.getMachines().stream()
				.map(machine -> toMachineDTO(machine, siteDTOWithoutMachines)).collect(Collectors.toSet());

		return new SiteDTOWithMachines(site.getId(), site.getSiteName(), toUserDTO(site.getVerantwoordelijke()),
				machineDTOs, site.getStatus(), toAddressDTO(site.getAddress()));
	}
	...
	public static List<SiteDTOWithMachines> toSiteDTOsWithMachines(List<Site> sites)
	{
		...
		return sites.stream().map(DTOMapper::toSiteDTOWithMachines).collect(Collectors.toList());
	}
	...
	public static MachineDTO toMachineDTO(Machine machine, SiteDTOWithoutMachines siteDTO)
	{
		...
		return new MachineDTO(machine.getId(), siteDTO, toUserDTO(machine.getTechnician()), machine.getCode(),
	...
	public static List<MachineDTO> toMachineDTOs(List<Machine> machines, SiteDTOWithoutMachines siteDTO)
	{
		...
		return machines.stream().map(machine -> toMachineDTO(machine, siteDTO)).collect(Collectors.toList());
	}
	...
	public static ReportDTO toReportDTO(Report report)
	{
		...
		return new ReportDTO(report.getMaintenance(), report.getSite(), report.getTechnician(), report.getStartDate(),
				report.getStartTime(), report.getEndDate(), report.getEndTime(), report.getReason(),
				report.getRemarks());
	}
