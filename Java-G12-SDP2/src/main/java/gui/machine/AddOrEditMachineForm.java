...
public class AddOrEditMachineForm extends VBox
{
	...
	private void initializeGUI()
	{
		...
		saveButton.setOnAction(e -> {
			try
			{
				...
				if (machineDTO != null)
				{
					...
					updatedMachine = new MachineDTO(machineDTO.id(), // Use existing machine's ID (so we update the
																		// correct one)
							selectedSite, selectedTechnician, code, machineDTO.status(), // Retain the existing status
																							// or modify if needed
							machineDTO.productieStatus(), // Retain the existing production status
							location, productInfo, machineDTO.lastMaintenance(), // Keep the same last maintenance
				...
				} else
				{
					...
					updatedMachine = new MachineDTO(0, // New machine, so ID is 0
							selectedSite, selectedTechnician, code, "Actief", // Default status?
							"Productie OK", // Default productieStatus?
							location, productInfo, LocalDateTime.now(), // Last maintenance = now
		...
		cancelButton.setOnAction(e -> mainLayout.showMachineScreen());

		HBox buttonBox = new HBox(10, cancelButton, saveButton);
		buttonBox.setAlignment(Pos.CENTER);

		this.getChildren().addAll(title, codeField, locationField, siteCb, techniekerCb, productInfoField,
				futureMaintenancePicker, buttonBox);
	}
