...
public class AddOrEditMaintenance extends AddOrEditAbstract
{
    ...
    protected void initializeFields()
    {
        ...
        if(AuthenticationUtil.hasRole(Role.TECHNIEKER)) {
            statusses = Arrays.stream(MaintenanceStatus.values())
                        .filter(s -> !s.equals(MaintenanceStatus.INGEPLAND))
                        .map(Enum::toString)
                        .toList();
        }
        ...
        machineMap = machines.stream().collect(Collectors.toMap(
            m -> String.format("%d Machine %s", m.id(), m.code()), 
            m -> m
        ));
        ...
        if (AuthenticationUtil.hasRole(Role.ADMINISTRATOR) || AuthenticationUtil.hasRole(Role.VERANTWOORDELIJKE))
        {
            try
            {
                ...
                LocalDateTime endDateTime = (execDate != null && endTime != null) 
                        ? LocalDateTime.of(execDate, endTime)
                        : null;
                ...
                if (isNew) {
                    maintenanceController.createMaintenance(execDate, startDateTime, endDateTime, technicianId, reasonField.getText(),
                            commentsField.getText(), status, machineId);
                } else {
                    maintenanceController.updateMaintenance(maintenanceDTO.id(), execDate, startDateTime, endDateTime, technicianId,
    ...
    {
        // Deze methode wordt niet gebruikt omdat we de handleInformationRequiredException 
        // methode hebben overschreven met een eigen implementatie die veldspecifieke labels gebruikt
    }
