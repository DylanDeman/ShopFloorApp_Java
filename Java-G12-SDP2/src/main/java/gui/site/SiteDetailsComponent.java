...
public class SiteDetailsComponent extends VBox implements Observer {
    ...
    public SiteDetailsComponent(MainLayout mainLayout, int siteId) {
        ...
        // Initialize the table with proper configuration
        ...
        // For debugging
        System.out.println("SiteDetailsComponent initialized for site ID: " + siteId);
        System.out.println("mahines of this site:" + site.machines());
    }
    ...
    private void initializeGUI() {
        ...
        allMachines = new ArrayList<>(); // Initialize to empty list
        filteredMachines = new ArrayList<>(); // Initialize to empty list
    ...
    private void configureTableLayout() {
        // Make sure the table is visible and has a proper size
        table.setMinHeight(300);  // Minimum height
        table.setPrefHeight(500); // Preferred height
        table.setMaxHeight(Double.MAX_VALUE); // Allow growing
        
        VBox.setVgrow(table, Priority.ALWAYS); // Let it grow in the VBox
    }
    ...
    private VBox createTitleSection() {
        ...
        HBox informationBox2 = new CustomInformationBox("Verantwoordelijke: %s %s"
                .formatted(verantwoordelijke.firstName(), verantwoordelijke.lastName()));
    ...
    private VBox createTableSection() {
        ...
        // Create and configure all table columns
        createTableColumns();
        
        // Make columns take up proportional space
        ...
        // Make table use all available width
    ...
    private void createTableColumns() {
        ...
        productionStatusColumn.setCellValueFactory(data -> 
            new SimpleStringProperty(data.getValue().productionStatus().toString()));
    ...
    private void filterTable() {
        ...
        System.out.println("Filtering with: " + 
                     ...
                     "prodStatus='" + selectedProductionStatus + "', " +
        ...
        List<MachineDTO> newFilteredList = allMachines.stream().filter(machine -> {
            ...
            boolean matchesSearch = searchQuery.isEmpty() || 
                    machine.location().toLowerCase().contains(searchQuery) ||
                    ...
                    (machine.technician() != null && 
                     machine.technician().firstName() != null &&
    ...
    private void updateTableItems() {
        ...
        if (filteredMachines.isEmpty()) {
            // When no machines are available, set an empty observable list
            ...
            // Set the table items with a new observable list
    ...
    public void update() {
        Platform.runLater(() -> {
            try {
                System.out.println("Observer update() called - refreshing site data");
                SiteDTOWithMachines updatedSite = sc.getSite(siteId);
                allMachines = updatedSite.machines().stream().toList();
                filteredMachines = new ArrayList<>(allMachines);
                updateFilterOptions();
                updateTable(filteredMachines);
                System.out.println("Observer update complete - refreshed " + allMachines.size() + " machines");
            } catch (Exception e) {
                System.err.println("Error in observer update: " + e.getMessage());
                e.printStackTrace();
            }
        });
    }
